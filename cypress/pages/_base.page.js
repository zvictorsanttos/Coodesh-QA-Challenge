export default class Base {
  static getElement(element, index = undefined) {
    ...
    if (typeof index !== "undefined" || index > 0) {
      elem = cy
        .get(element, { timeout: Cypress.env("global_timeout") })
        .eq(index);
    } else {
  ...
  static getElementContaining(text) {
    return cy
      .contains(text, { timeout: Cypress.env("global_timeout") })
      .should("be.visible");
  }
  ...
  static getElementByXPath(element, index) {
    ...
    if (typeof index !== "undefined" || index > 0) {
      elem = cy
        .xpath(element, { timeout: Cypress.env("global_timeout") })
        .eq(index);
    } else {
  ...
  static typeWithDay(element, value) {
    this.getElement(element).type(value, {
      timeout: Cypress.env("global_timeout"),
    });
  }
  ...
  static verifyIfElementIsHidden(element) {
    this.getElement(element).should("not.be.visible", {
      timeout: Cypress.env("global_timeout"),
    });
  }
